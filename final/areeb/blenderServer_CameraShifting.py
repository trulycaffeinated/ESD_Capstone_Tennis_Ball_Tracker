...
def xform_stereo_rig(x, y, z, pitch, roll, yaw):
    ...
    # Save current stereo baseline
    offset = camR.location - camL.location

    pitchRad = math.radians(pitch)
    rollRad = math.radians(roll)
    yawRad = math.radians(yaw)
    rot = Euler((pitchRad, rollRad, yawRad), 'XYZ')

    # Move left camera
    camL.location = (x, y, z)
    camL.rotation_euler = rot

    # Move right camera, keep same baseline
    camR.location = camL.location + offset
    camR.rotation_euler = rot
...
def handle_data():
    ...
    # move stereo camera rig instead of moving the ball
    xform_stereo_rig(floats[2], floats[3], floats[4],
    ...
    # send ball location back if needed
